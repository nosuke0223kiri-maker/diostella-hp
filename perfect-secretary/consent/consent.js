...
function render(info) {
  ...
  if (info.nickname) $("ask-who").textContent = `Diostella（ディオステラ）の「完璧秘書」アプリをご利用のお子さま（アプリでの呼び名：${info.nickname}）から、保護者の方の同意のお願いが届いています。`;
  if (info.minAge) $("ask-law").textContent = `完璧秘書では、${info.minAge}歳未満の方が秘書チャットのAI機能をお使いになる際、法律（個人情報の保護に関する法律 第40条の2）にもとづき、保護者（親権者など）の方の同意をいただいています。`;
